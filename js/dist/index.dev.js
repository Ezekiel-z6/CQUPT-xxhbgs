...
window.onload = function () {
  ...
  function setActive(index) {
    $(bannerContent).eq(index).addClass("active").siblings().removeClass("active");
    $(bannerSelection).eq(index).addClass("active").siblings().removeClass("active");
  }
  ...
  $.each(itNavList, function (indexInArray, valueOfElement) {
    $('<div class="card"></div>').append('<div class="title">' + itNavList.eq(indexInArray).text() + "</div>").append('<ul class="btns">' + itBtnList.eq(indexInArray).html() + "</ul>").appendTo(panel);
  }); // 自动生成移动端新闻动态
  ...
  newsPanel.append('<div class="bg-title"><div>' + newsTitleList.eq(0).text() + "</div></div>").append($('<div class="card"></div>').append(newsPic).append(newsContent.eq(0)).append(newsMoreList.eq(0))).append('<div class="bg-title"><div>' + newsTitleList.eq(1).text() + "</div></div>").append($('<div class="card"></div>').append(newsContent.eq(1)).append(newsMoreList.eq(1)));
};